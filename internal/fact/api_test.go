...
func TestCreateFactAPIEndpoint(t *testing.T) {
	...
	tests := []test.APITestCase{
		...
		},
		{
			...
			Method:       "POST",
			URL:          "/apps/app_id/connections/conn_id/facts",
			Body:         `{"facts":[{"key": "key", "value":"value", "group": {"name":"group1"}, "type": "password"}]}`,
			Header:       nil,
			WantStatus:   http.StatusAccepted,
			WantResponse: ``,
		},
		{
			...
			Method:       "POST",
			URL:          "/apps/app_id/connections/conn_id/facts",
			Body:         `{"facts":[{"key": "key", "value":"value", "group": {"name":"group1"}, "type": "invalid"}]}`,
			Header:       nil,
			WantStatus:   http.StatusBadRequest,
			WantResponse: `{"details":"type: must be a valid value.", "error":"Invalid input", "status":400}`,
		},
		{
			Name:         "invalid group",
			Method:       "POST",
			URL:          "/apps/app_id/connections/conn_id/facts",
			Body:         `{"facts":[{"key": "key", "value":"value", "group": {"name":""}, "type": "password"}]}`,
			Header:       nil,
			WantStatus:   http.StatusBadRequest,
